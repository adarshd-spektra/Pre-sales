...
## Task 1: Sign a release approval and issue a certificate
...
1. Run the following commands to return to your working directory and confirm your keys are present.
    ...
    **Expected Output:**
    ...
    -rw-r--r-- 1 azureuser azureuser  451 Aug 11 09:28 public_key.pem
    ```

    >**Note:** Your file sizes may differ by a few bytes because base64 encoding of your particular key is not a fixed length. Notice the permissions OpenSSL chose: the private key is `-rw-------`, readable only by you, while the public key is world-readable. The tool defends the distinction for you.
...
1. Run the following command to inspect the signature you produced.
    ...
    **Expected Output:**
    ...
    ```output
    -rw-r--r-- 1 azureuser azureuser 256 Aug 11 09:45 message.sig
    ```
...
1. Run the following command to verify the **original** signature against the **forged** file.
    ...
    **Expected Output:**
    ...
    ```output
    40B7A2CF877F0000:error:02000068:rsa routines:ossl_rsa_verify:bad signature:../crypto/rsa/rsa_sign.c:441:
    40B7A2CF877F0000:error:1C880004:Provider routines:rsa_verify_directly:RSA lib:../providers/implementations/signature/rsa_sig.c:1035:
    Verification failure
    ```

    >**Note:** The hexadecimal prefixes and file paths will differ on your run, and note that **`Verification failure` is printed last, after the error detail** — do not stop reading at the first line. The verdict is what matters: the signature no longer matches.
    >
